...
- 📐 원본 비율 유지하면서 1024x1024 크기로 변환
- 🎨 흰색 배경에 이미지 중앙 배치
- 🚀 디렉토리 내 모든 이미지 일괄 처리
- ⚙️ 커스터마이징 가능한 크기 및 품질 설정
...
**모든 옵션 조합:**
```bash
python image_thumbnail_converter.py -i ./images -o ./output -s 800 800 -q 90
```
...
3. 원본 비율을 유지하면서 크기 조정
4. 1024x1024 흰색 캔버스에 중앙 배치
5. JPEG 형식으로 최적화하여 저장
